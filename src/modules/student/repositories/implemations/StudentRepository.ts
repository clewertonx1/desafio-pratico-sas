...
class StudentRepository implements IStudentRepository {
  ...
  async startSimulated({ simulated, studentId }): Promise<Student> {
    ...
    const studentSimulated = this.studentSimulatedRepository.create({
      simulatedId,
      tests: [],
    });

    await simulated.tests.forEach(async (test) => {
      const studentTest = await this.studentTestRepository.create({
        testId: test.id,
        quests: [],
      });
      await test.questions.forEach(async (quest) => {
        const studentQuest = await this.studentQuestRepository.create({
          questId: quest.id,
          response: "",
        });
  ...
  async findSimulatedInStudent({ studentId, simulatedId }) {
    const simulated = await this.studentSimulatedRepository.findOne({
  ...
  async calculateAndSaveTestScore({
    studentId,
    simulatedId,
    testId,
    ...
    const Vd = 8;
    const score =
      countEasyQuestsRight * Vf +
      coutMediumQuestRight * Vm +
      coutDificultyQuestRigth * Vd +
      600;
  ...
  async checkAllQuestionHaveBeenAnswered({ studentId, simulatedId, testId }) {
    ...
    await Promise.all(
      student.simulateds.map(async (simulated) => {
        if (simulated.simulatedId == simulatedId) {
          await Promise.all(
            simulated.tests.map(async (test) => {
              if (test.testId == testId) {
                await Promise.all(
                  test.quests.map(async (quest) => {
                    if (quest.done) {
                      countAlreadyAnswerQuests = countAlreadyAnswerQuests + 1;
                      const { right, dificulty } =
                        await this.verifyQuestReponse({
    ...
    console.log({
      countAlreadyAnswerQuests,
      countMissingAnswerQuests,
      countEasyQuestsRight,
      coutMediumQuestRight,
      coutDificultyQuestRigth,
    });
  ...
  async getRankingOfSimulated({ simulatedId }): Promise<any> {
    const simulateds = await this.studentSimulatedRepository.find({
      relations: ["student"],
      order: { score: "ASC" },
      take: 5,
    });
    const students = simulateds.map((simulated) => {
      return simulated.student;
    });
    return students;
  }
